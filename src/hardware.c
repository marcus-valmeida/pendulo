 ...
 * --------------------------------------------------------------------------*/
static void MX_TIM3_Init(void) {
    TIM_OC_InitTypeDef sConfigOC = {0};                                         // Cria formulario do canal PWM.

    __HAL_RCC_TIM3_CLK_ENABLE();                                                // Habilita o Timer 3.
    __HAL_RCC_GPIOA_CLK_ENABLE();                                               // Habilita a porta A.

    htim3.Instance               = TIM3;                                        // Define o uso do Timer 3.
    htim3.Init.Prescaler         = 72 - 1;                                      // 72 MHz / 72 = 1 MHz
    htim3.Init.CounterMode       = TIM_COUNTERMODE_UP;                          // Contador crescente.
    htim3.Init.Period            = 1000 - 1;                                    // 1 MHz / 1000 = 1 kHz
    htim3.Init.ClockDivision     = TIM_CLOCKDIVISION_DIV1;                      // Sem divisao do clock.
    htim3.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_DISABLE;              // Sem preload.
    if (HAL_TIM_PWM_Init(&htim3) != HAL_OK) { Error_Handler(); }

    sConfigOC.OCMode     = TIM_OCMODE_PWM1;                                     // Modo PWM padrao (pulso ativo ate o limite).
    sConfigOC.Pulse      = 0;                                                   // comeca em 0% (motor desligado)
    sConfigOC.OCPolarity = TIM_OCPOLARITY_HIGH;                                 // Pulso ativo em nivel alto.
    sConfigOC.OCFastMode = TIM_OCFAST_DISABLE;                                  // Modo rapido desligado.
    ...
    GPIO_InitTypeDef GPIO_InitStruct = {0};                                     // Cria formulario de configuracao.
    GPIO_InitStruct.Pin   = GPIO_PIN_6;                                         // Define o pino 6 (saida do PWM).
    GPIO_InitStruct.Mode  = GPIO_MODE_AF_PP;                                    // Funcao alternativa, push-pull.
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;                               // Velocidade alta de chaveamento.
    ...
    HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_1);                                   // Liga a geracao do sinal PWM.
}
 ...
 * --------------------------------------------------------------------------*/
static void MX_ADC1_Init(void) {
    ADC_ChannelConfTypeDef sConfig = {0};                                       // Cria formulario do canal ADC.

    __HAL_RCC_ADC1_CLK_ENABLE();                                                // Habilita o periferico ADC1.
    __HAL_RCC_GPIOA_CLK_ENABLE();                                               // Habilita a porta A.

    // PA4 em modo entrada analogica (sem pull-up/down, sem digital).
    GPIO_InitTypeDef GPIO_InitStruct = {0};                                     // Cria formulario de configuracao.
    GPIO_InitStruct.Pin  = GPIO_PIN_4;                                          // Define o pino 4 (entrada do wiper).
    GPIO_InitStruct.Mode = GPIO_MODE_ANALOG;                                    // Modo analogico (desliga logica digital do pino).
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);                                     // Entrega o formulario para o HAL.

    hadc1.Instance                   = ADC1;                                    // Define o uso do periferico ADC1.
    hadc1.Init.ScanConvMode          = DISABLE;                                 // So um canal, nao precisa varrer varios.
    hadc1.Init.ContinuousConvMode    = DISABLE;                                 // So converte quando mandado.
    hadc1.Init.DiscontinuousConvMode = DISABLE;                                 // Nao aplica (so ha um canal mesmo).
    hadc1.Init.DataAlign             = ADC_DATAALIGN_RIGHT;                     // Resultado alinhado a direita (LSB no bit 0).
    hadc1.Init.NbrOfConversion       = 1;                                       // Uma unica conversao por chamada.
