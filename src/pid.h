/**
  ...
  *          LE do aeropendulo.c (angulo) e ESCREVE no motor.c (PWM).
  *
  *  ESTRATEGIA:
  *    - PID completo (P + I + D) para evitar os "supapos" (oscilacao).
  *    - Termo D com filtro (N) freia o pendulo perto do alvo.
  *    - Anti-windup no integral evita acumulo de erro e solavancos.
  *    - Rampa suave no setpoint: o alvo se move devagar ate o desejado.
  *    - Seguranca em 3 estagios (em modulo, vale p/ + e -):
  *        |ang| >= 120  -> corta 20% da potencia atual
  *        |ang| >= 150  -> desliga o motor
...
/* Getters para exibir no display / depurar. */
